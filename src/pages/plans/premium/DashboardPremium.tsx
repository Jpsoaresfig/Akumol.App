    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 p-4 md:p-8 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <header className="max-w-7xl mx-auto flex justify-between items-center mb-10">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            {/* Badge com cor exclusiva para o Premium */}
            <span className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest shadow-sm">
              PLANO PREMIUM
            </span>
            <p className="text-slate-500 dark:text-slate-400 text-xs font-medium italic">Varredura Oculta Ativa</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* RECURSO DE DESTAQUE (PREMIUM): AGENTE SOMBRA */}
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2.5rem] shadow-sm relative overflow-hidden transition-colors duration-300">
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4 text-red-500">
                <Zap size={18} />
                <span className="text-xs font-black uppercase tracking-widest">Agente Sombra</span>
              </div>
              <h2 className="text-slate-800 dark:text-white font-black text-3xl tracking-tight">Vazamentos Ocultos</h2>
              <div className="flex items-baseline gap-2 mt-2">
                <span className="text-6xl font-black tracking-tighter text-red-500">R$ 142,90</span>
                <span className="text-xl font-bold text-slate-400 dark:text-slate-500">/mês</span>
              </div>
              <p className="text-slate-500 dark:text-slate-400 mt-4 max-w-md text-sm leading-relaxed font-medium">
                A IA analisou o seu Open Banking e encontrou assinaturas esquecidas e taxas abusivas. Clique abaixo para contestar e cancelar.
              </p>
              <button className="mt-8 bg-red-500 text-white font-bold py-3 px-8 rounded-xl text-sm hover:bg-red-600 transition-all shadow-lg shadow-red-500/30 dark:shadow-none">
                Exterminar Gastos
              </button>
            </div>
            {/* Fundo decorativo vermelho */}
            <div className="absolute top-0 right-0 w-72 h-72 bg-red-50 dark:bg-red-900/10 rounded-full -mr-24 -mt-24 z-0 opacity-60"></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            
            {/* RECURSO ATIVO (BASIC/PREMIUM): MONITORIZAÇÃO */}
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-[2rem] shadow-sm transition-colors duration-300">
              <div className="flex justify-between items-start mb-6">
                <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-500 rounded-2xl"><CreditCard size={24} /></div>
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Open Banking</span>
              </div>
              <h3 className="font-bold text-slate-800 dark:text-white text-sm">Monitorização Ativa</h3>
              <p className="text-2xl font-black text-slate-800 dark:text-white mt-1">Conectado</p>
              <p className="text-xs text-slate-400 mt-2 font-medium">A analisar o padrão de consumo.</p>
            </div>

            {/* RECURSO BLOQUEADO (PLUS PRO): ARQUITETO DE HERANÇA */}
            <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-800 p-6 rounded-[2rem] relative overflow-hidden group transition-colors duration-300">
              ...
              <h3 className="font-bold text-slate-800 dark:text-white text-sm opacity-40">Arredondamento Automático</h3>
