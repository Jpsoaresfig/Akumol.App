...
import { Ghost, ChevronLeft, Plus, AlertTriangle, } from 'lucide-react';
...
const AgenteSombra: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6">
      ...
      <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-8 rounded-r-2xl">
        <div className="flex items-center gap-3">
          <AlertTriangle className="text-red-600" />
          <p className="text-sm text-red-800 font-medium">
            O Agente Sombra detectou que você não utiliza a <strong>Netflix</strong> há mais de 20 dias. Deseja cancelar?
          </p>
        </div>
      </div>
        ...
        {assinaturas.map((sub) => (
          <div key={sub.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              {sub.statusUso === 'baixo' && (
                <button className="text-[10px] font-bold text-red-500 uppercase mt-1">CORTAR GASTO</button>
              )}
            </div>
          </div>
        ))}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-6 z-50">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl w-full max-w-sm border border-slate-200 dark:border-slate-800">
            <h2 className="text-xl font-black mb-6">Nova Assinatura</h2>
            <input 
              type="text" placeholder="Nome (ex: Spotify)" 
              className="w-full p-4 rounded-xl bg-slate-100 dark:bg-slate-800 mb-4 outline-none focus:ring-2 ring-indigo-500"
              value={novoNome} onChange={(e) => setNovoNome(e.target.value)}
            />
            <input 
              type="number" placeholder="Valor (R$)" 
              className="w-full p-4 rounded-xl bg-slate-100 dark:bg-slate-800 mb-6 outline-none focus:ring-2 ring-indigo-500"
              value={novoValor} onChange={(e) => setNovoValor(e.target.value)}
            />
            <div className="flex gap-2">
              <button onClick={() => setShowAdd(false)} className="flex-1 p-4 font-bold text-slate-400">Cancelar</button>
